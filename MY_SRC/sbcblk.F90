...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist' )
   ...
   SUBROUTINE blk_oce_1( kt, pwndi, pwndj, ptair, pqair,         &  ! inp
      ...
      IF( ln_stopack .AND. nn_spp_blkd > 0 ) THEN
                 CALL spp_gen(kt, zcd_oce, nn_spp_blkd, rn_blkd_sd, jk_spp_blkd )
      ENDIF
      IF( ln_stopack .AND. nn_spp_blkh > 0 ) THEN
                 CALL spp_gen(kt, zch_oce, nn_spp_blkh, rn_blkh_sd, jk_spp_blkh )
      ENDIF
      IF( ln_stopack .AND. nn_spp_blke > 0 ) THEN
                 CALL spp_gen(kt, zce_oce, nn_spp_blke, rn_blke_sd, jk_spp_blke )
      ENDIF
   ...
   SUBROUTINE blk_ice_1( pwndi, pwndj, ptair, pqair, pslp , puice, pvice, ptsui,  &   ! inputs
      ...
      IF( ln_stopack .AND. nn_spp_blkd > 0 ) THEN
                 CALL spp_gen(0, Cd_ice , nn_spp_blkd, rn_blkd_sd, jk_spp_blkd )
      ENDIF
      IF( ln_stopack .AND. nn_spp_blkh > 0 ) THEN
                 CALL spp_gen(0, Ch_ice , nn_spp_blkh, rn_blkh_sd, jk_spp_blkh )
      ENDIF
      IF( ln_stopack .AND. nn_spp_blke > 0 ) THEN
                 CALL spp_gen(0, Ce_ice , nn_spp_blke, rn_blke_sd, jk_spp_blke )
      ENDIF
