...
# endif
      ...
      IF( ln_skeb  )     CALL skeb_comp( kstp, Nbb, Nnn, Nrhs )                       ! SKEB perturbations
      ...
      IF( ln_sppt_dyn )  CALL dyn_sppt_apply ( kstp, 0,  Nbb, Nnn      , uu, vv, Nrhs  )
      ...
      IF( ln_sppt_dyn )     CALL dyn_sppt_apply ( kstp, 1,  Nbb, Nnn      , uu, vv, Nrhs  )
      IF( ln_skeb .AND. ln_skeb_apply .and. ABS( nn_skst ) .eq. 3) CALL skeb_apply ( kstp,  Nbb, Nnn, Nrhs )
      ...
      IF( ln_dynspg_ts ) THEN                                                       ! vertical scale factors and vertical velocity need to be updated
         ...
         IF( ln_zad_Aimp )  CALL wAimp      ( kstp,      Nnn )                      ! Adaptive-implicit vertical advection partitioning
      ENDIF
      IF( ln_skeb .AND. ln_skeb_apply .and. ABS( nn_skst ) .eq. 1) CALL skeb_apply ( kstp,  Nbb, Nnn, Nrhs )
      ...
      IF( ln_sppt_tra )  CALL tra_sppt_apply ( kstp, 0, Nbb, Nnn, ts, Nrhs )
      ...
      DO jtile = 1, nijtile
         ...
         IF( lrst_oce .AND. ln_stopack) CALL stopack_rst( kstp, 'WRITE' )
                            ...
                            IF( ln_sppt_tra )  CALL tra_sppt_apply ( kstp, 1, Nbb, Nnn, ts, Nrhs )
...
!!
      ...
      ENDIF
      !
      IF( ln_sppt_tra )  CALL tra_sppt_apply ( kstp, 2, Nbb, Nnn, ts, Nrhs )
      IF( ln_sppt_dyn )  CALL dyn_sppt_apply ( kstp, 2,  Nbb, Nnn      , uu, vv, Nrhs  )
      IF( ln_skeb .AND. ln_skeb_apply .and. ABS(nn_skst) .eq. 2) CALL skeb_apply ( kstp, Nbb, Nnn, Nrhs  )
      ! Swap time levels
      Nrhs = Nbb
      Nbb = Nnn
      Nnn = Naa
