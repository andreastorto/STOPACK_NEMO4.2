...
!
         ...
         DO_3D_OVR( nn_hls, nn_hls, nn_hls, nn_hls, 2, nksr + 1 )
            ...
            zc0 = ze0(ji,jj) * EXP( - ze3t * xsi0r(ji,jj) )
      ...
      CASE( np_2BD  )            !==  2-bands fluxes  ==!
         ...
         DO_3D_OVR( nn_hls, nn_hls, nn_hls, nn_hls, 1, nksr )          !* now qsr induced heat content
            zc0 = zz0 * EXP( -gdepw(ji,jj,jk  ,Kmm)*xsi0r(ji,jj) ) + zz1 * EXP( -gdepw(ji,jj,jk  ,Kmm)*xsi1r )
            zc1 = zz0 * EXP( -gdepw(ji,jj,jk+1,Kmm)*xsi0r(ji,jj) ) + zz1 * EXP( -gdepw(ji,jj,jk+1,Kmm)*xsi1r )
